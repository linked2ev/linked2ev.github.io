...
정의 된 메서드 | 설명
--- | ---
`int available()` | - `현재 읽을 수 있는 바이트 수를 얻음`
...
long skip(long n) | - inputStream 에서 n 바이트 만큼 데이터를 스킵하고 바이트 수 반환
...
정의 된 메서드 | 설명
--- | ---
`void close()` | - OutputStream 을 닫음
`void flush()` | - `버퍼에 남은 출력 스트림을 출력`
abstract void write(int i) | - 정수 i의 하위 8비트를 출력
`void write(byte buf[])` | - `buf 의 내용을 출력`
`void write(byte buf[], int index, int size)` | - `buf 의 index 위치부터 size만큼의 바이트를 출력`
...
--- | ---
FileInputStream(String filePath) | - filePath로 지정한 파일에 대한 입력 스트림 생성
FileInputStream(File fileObj) | - fileObj로 지정한 파일에 대한 입력 스트림 생성
FileUnputStream(FileDescriptor fdObj) | - fdObj 로 기존의 접속을 나타내는 파일 시스템의 입력 스트림 생성
...
--- | ---
FileOutputStream(String filePath) | 지정한 파일로 OutputStream 을 생성
FileOutputStream(String filePath, boolean append) | - 지정한 파일로 OutputStream 을 생성, append 할 때 append 모드 설정
FileOutputStream(File fileObj) | - fileObj로 지정된 파일에 대한 OutputStream 을 생성
FileOutputStream(File fileObj) | - fileObj로 지정된 파일에 대한 OutputStream 을 생성, append 할 때 append 모드 설정
FileOutputStream(FileDescriptor fdObj) | - fdObj 로 기존의 접속을 나타내는 파일 시스템의 출력 스트림 생성
...
메서드/생성자 | 설명
--- | ---
`BufferedInputStream(inputStream in, int size)` | - 주어진 InputStream 인스턴스를 입력소스(Input source)로 지정된 크기(byte단위)의 버퍼를 갖는 BufferedInput Stream 인스턴스 생성 
