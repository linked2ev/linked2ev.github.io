...
| **`실체화`<br>(Realization)** | 인터페이스와 이를 구현하는 클래스 간의 관계 ("IS-IMPLEMENTED-BY" 관계)<br>- 책임들의 집한인 인터페이스와 이 책임들을 실제로 실현한 클래스들 사이의 관계           | - 인터페이스에 정의된 메서드를 구현<br>- 인터페이스와 구현 클래스 간의 관계를 표현                   | 점선 삼각형 (---▷)      |
