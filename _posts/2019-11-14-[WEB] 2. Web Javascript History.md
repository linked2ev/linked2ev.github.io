...
우선 작성자가 아직 코린이고 jQuery부터 학습한 비전공자이기에 많은 javascript 라이브러리 직접적인 프로젝트 개발은 많지는 않고 겪은 내용은 아니다. 하지만 중국어를 공부 할 때도 이상하게 회화보단 문법을 좋아했는지 이런 과정(?)에 가끔 재미를 느낀다.
...
그래서 우리 흔히 쓰는 버전이 ES5, 최근에 let, const, Arrow 함수 등이 ES6이다. 
...
## 5. Front-End Framework, 프론트 프레임워크
---
